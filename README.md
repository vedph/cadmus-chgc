...
- `IMAGE_HID` = the human-friendly image ID, built with the manuscript ID + `/` + the image ordinal number from its IIIF source, e.g. `ccc-ms029/1`.
- `IMAGE_URI`: the source image URI.
- `ANNOTATION_ID`: the annotation GUID (prefixed with `#`, as generated by Recogito Annotorious library), e.g. `#36c9730c-a7c9-4a28-8889-8d6015ee14fe`.
...
- `SVG` is the SVG code, produced by Annotorious for non-rectangular shapes, and supplied by the exporter for rectangular shapes.
